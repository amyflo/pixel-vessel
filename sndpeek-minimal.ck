...
// color0
waveform.mat().color( @(1, 1, 1) );

// spectrum renderer
// Glines spectrum --> GG.scene(); 
// spectrum.mat().lineWidth(1.0);
// // translate down
...
MangoUVMaterial normalMat;
normalMat.polygonMode(Material.POLYGON_LINE);
SphereGeometry latheGeo;
...
//     5 => int segments;
//     0 => float thetaStart;
//     Math.PI * 1 => float thetaLength;
//     while (true)
//     {
//         Math.sin(now / second) *Math.PI + Math.PI => thetaLength;
//         latheGeo.set(radius, segments, Math.random2f(thetaStart, thetaLength), thetaLength);
//         GG.nextFrame() => now;
//     }
// }
// spork ~circleSetter();
...
//     {
//         Math.random2f(0, 3) => float x_rand;
...
//     //     // Math.sin(.3 * ftime) * Math.PI + Math.PI - (Math.random2f(0, 2)) => phiLength;
...
fun void map2waveform( float in[], vec3 out[] )
{
    ...
    for( auto s : in )
    {
        ...
        wireMesh.set(latheGeo, normalMat);
        wireMesh.position(@(-width/2 + width/WINDOW_SIZE*i, s * 6 * window[i], 0));
        wireMesh  --> GG.scene();
        // increment
        i++;
    }
...
// // map audio buffer to 3D positions
// fun void drawWaveform(float in[])
// {
//     // mapping to xyz coordinate
//     int i;
//     DISPLAY_WIDTH => float width;
